...
<script>
	$(document).ready(function(){
		...
		if(<%=session.getAttribute("rname")%> == null){
			...
			location.href = "/client/login?category=list";
		}
...
<script>
	...
	/* $(".move").on("click",function(e){
        ...
        actionForm.attr("action","/client/read").submit();
		...
		<c:forEach var="request" items="${list}">
			<tr>
				<form action="/client/cancel" method="post">
					<td class="align-middle"><c:out value="${request.rn}"/><input type="hidden" name="rid" value="${request.rid}"></td>
					...
					<td class="align-middle">
						<a class="move" href='<c:out value="${request.rid}"/>' style="text-decoration: none;">
							<c:choose>
					...
					<td class="align-middle"><c:out value="${request.hopeDate}"/></td>
					<td class="align-middle">
						<c:if test="${request.status eq 'A'}">
							접수 대기<br><button type="submit" class="btn btn-outline-dark btn-sm" style="font-size: 11px">취소</button>
						</c:if>
						<c:if test="${request.status eq 'S'}">
							접수 완료<br><a class="btn btn-outline-dark btn-sm" style="font-size: 11px" onclick="window.open('${path}/client/info?fid=${request.fid}', '_blank', 'width=800px,height=300px,scrollbars=no,resizable=no'); return false;">기사 정보</a>
							<input type="hidden" name="fid" value='<c:out value="${request.fid}"/>'>
						</c:if>
						<c:if test="${request.status eq 'D'}">
							수리 완료<br><a href="/client/fixList?rid=${request.rid}&fid=${request.fid}" class="btn btn-outline-dark btn-sm" style="font-size: 11px">내역보기</a>
						</c:if>
